...
function exibirFraseInformativa(url) {
    ...
    <p>Esses são uns filmes aleatorios que eu dei nota. <br>
    Meu filme favorito foi <strong>A Espera De Um Milagre</strong>, com um total de <span>${valores[6]}</span> pontos.</p>
    <p>Em segundo lugar,empatados temos: <strong>As Branquelas</strong>, que teve <span>${valores[1]}</span> pontos, <strong>Gente Grande</strong> com <span>${valores[3]}</span> pontos.</p>
    E <strong>Ratatouille</strong> com <span>${valores[5]}</span> pontos.</p>
    <p>Os que menos gosto são: <br>
    <strong>Vovózona</strong> com <span>${valores[0]}</span> pontos, <br>
    <strong>Norbit</strong> com <span>${valores[2]}</span> pontos, e <br>
    <strong>Carros 2</strong> com <span>${valores[4]}</span> pontos.</p>
    `;
}